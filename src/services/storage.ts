...
export async function saveState(tableName: string, data: Record<string, unknown>): Promise<void> {
	try {
...
export async function loadState(tableName: string): Promise<Record<string, unknown> | undefined> {
	try {
		const db = await openDB();
