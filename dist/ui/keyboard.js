...
export function handleKeyboard(e, args) {
    const { selection, totalRows, totalCols, isEditing } = args;
    if (isEditing)
        return; // в режиме редактирования клавиши идут в инпут
    ...
    switch (e.key) {
        case "ArrowUp":
            next = { row: cursor.row - 1, col: cursor.col };
            break;
        case "ArrowDown":
            next = { row: cursor.row + 1, col: cursor.col };
            break;
        case "ArrowLeft":
            next = { row: cursor.row, col: cursor.col - 1 };
            break;
        case "ArrowRight":
            next = { row: cursor.row, col: cursor.col + 1 };
            break;
        case "Enter":
            ...
            break;
        case "Tab":
            next = { row: cursor.row, col: cursor.col + (e.shiftKey ? -1 : 1) };
            break;
        case "F2":
            args.startEdit(cursor.row, cursor.col, "");
            e.preventDefault();
            return;
        case "Delete":
        case "Backspace":
    ...
    if (next.col < 0)
        next.col = 0;
    if (next.row >= totalRows)
        next.row = totalRows - 1;
    if (next.col >= totalCols)
        next.col = totalCols - 1;
    // Shift — расширение выделения, без Shift — перемещение курсора
    if (e.shiftKey && selection.start)
        args.setSelection({ start: selection.start, end: next });
    else
        args.setSelection({ start: next, end: next });
}
