...
import { BUFFER_COLS, BUFFER_ROWS, DEFAULT_COL_WIDTH, DEFAULT_ROW_HEIGHT, EXPAND_COLS_BY, EXPAND_ROWS_BY, HEADER_ROW_HEIGHT, HEADER_WIDTH, OVERSCAN_COLS, OVERSCAN_ROWS, } from "../utils/consts";
import { flattenColumns } from "../utils/column-tree";
export class Renderer {
    constructor(container, model, totalRows, totalCols, colWidth = DEFAULT_COL_WIDTH, rowH = DEFAULT_ROW_HEIGHT, columns = []) {
        this.totalRows = totalRows;
        this.totalCols = totalCols;
        this.headerRows = []; // строки шапки (многоуровневая)
        this.rows = []; // пул видимых строк (переиспользуются при скролле)
        this.headerGrid = []; // сетка ячеек шапки (из flattenColumns)
        this.maxDepth = 1; // глубина иерархии колонок
        this.headerH = 28; // общая высота шапки = maxDepth * HEADER_ROW_HEIGHT
        this.colLeftCache = []; // префикс-суммы: colLeft(col) = colLeftCache[col]
        this.rowHeights = []; // высота каждой строки (может различаться)
        this.rowTopCache = []; // префикс-суммы: rowTop(row) = rowTopCache[row]
        this.currentStartRow = 0; // текущее окно виртуализации
        this.currentEndRow = -1;
        this.currentStartCol = 0;
        this.currentEndCol = -1;
        ...
        this.colWidths = Array.from({ length: this.totalCols }, (_, i) => this.columns[i]?.width ?? colWidth);
    ...
    getRowHeights() { return [...this.rowHeights]; }
    ...
    findColByOffset(bodyX) {
        let remaining = bodyX;
        for (let c = 0; c < this.colWidths.length; c++) {
            if (remaining < this.colWidths[c])
                return c;
    ...
    // Private
    // ──────────────────────────────────────────────────────────────────────
    toggleHeaderClass(el, idx, axis) {
        if (!this.selectedRect?.start || !this.selectedRect?.end)
            return;
        if (axis === "col") {
    ...
    renderRowContents(pool, row, sc, ec) {
        const count = ec - sc + 1;
        // Grow cell pool
    ...
    layoutHeader(sc, ec) {
        ...
        const covered = new Set(); // key = "level,col"
        for (let level = 0; level < this.maxDepth; level++) {
            ...
            for (let i = 0; i < cells.length; i++) {
                ...
                let label = el.querySelector(".nt-header-label");
                if (!label) {
                    el.textContent = "";
    ...
    layoutRowHeader(sr, er) {
        ...
        for (let i = 0; i < needed; i++) {
            const r = sr + i;
            const el = this.headerCol.children[i];
            ...
            let rh = el.querySelector(".nt-resize-handle-row");
            if (!rh) {
        ...
        for (let i = needed; i < this.headerCol.children.length; i++) {
            this.headerCol.children[i].style.display = "none";
        }
    }
}
...
function renderCellContent(el, cell, colDef) {
    // Apply cell styles
