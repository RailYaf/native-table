...
export class NativeSheet {
    ...
    container, options) {
        ...
        this.renderer = new Renderer(container, this.model, options.rows, options.cols, options.defaultColWidth, options.defaultRowHeight, options.columns ?? []);
        // Применить начальные ширины/высоты/стили из IndexedDB
        if (options.initialWidths)
            for (const [c, w] of Object.entries(options.initialWidths))
                this.renderer.setColWidth(Number(c), w);
        if (options.initialHeights)
            this.renderer.setRowHeights(options.initialHeights);
        if (options.initialStyles)
            this.applyStyles(options.initialStyles);
        ...
        this.editor = new Editor(this.renderer.cellsLayer, this.model, (row, col) => this.renderer.bodyBox(row, col), (row, col, value, direction) => this.commitEdit(row, col, value, direction), () => this.cancelEdit());
    ...
    setSelection(rect) {
        ...
        this.renderer.selectedRect = rect;
        if (rect.end)
            this.renderer.scrollToCell(rect.end.row, rect.end.col);
        this.renderer.render();
        this.updateToolbarStyleState();
    }
    cellAtEvent(e) {
    ...
    onCheckboxClick(e) {
        if (e.target.classList.contains("nt-checkbox")) {
            e.preventDefault();
        }
    }
    onContextMenu(e) {
    ...
    showSortFilterPopup(col, clientX, clientY) {
        ...
        const sortDir = !existing ? "none" : existing.asc ? "asc" : "desc";
        ...
        this.sortFilterPopup.innerHTML = `
			...
			<div class="nt-sf-filter-menu-item">Тип фильтра<span class="nt-sf-op-arrow">▸</span></div>
        ...
        const filterMenuItem = this.sortFilterPopup.querySelector(".nt-sf-filter-menu-item");
        if (filterMenuItem) {
            const showTypesFlyout = () => {
                ...
                for (const op of filterOps) {
                    ...
                    item.addEventListener("click", (ev) => {
                        ev.stopPropagation();
                        const valBlock = this.sortFilterPopup.querySelector(".nt-sf-values-block");
                        const customBlock = this.sortFilterPopup.querySelector(".nt-sf-custom-block");
                        if (op.value === "values") {
                            if (valBlock)
                            ...
                            const val2 = this.sortFilterPopup.querySelector(`#sf-val2-${col}`);
                            if (val2)
                                val2.style.display = op.value === "between" ? "block" : "none";
                        }
                        filterMenuItem.dataset.op = op.value;
                        this.hideFilterChildPopup();
                        updateApplyBtn();
                    });
                    popup.append(item);
                ...
                const close = (ev) => {
                    if (popup.contains(ev.target) || filterMenuItem.contains(ev.target))
                        return;
                    this.hideFilterChildPopup();
        ...
        const getFilterType = () => {
            return (filterMenuItem?.dataset.op ?? "values");
        };
        const updateApplyBtn = () => {
            const ft = getFilterType();
            const apply = this.sortFilterPopup.querySelector(".nt-sf-btn--apply");
            let ok = false;
            if (ft === "values") {
                const cbs = this.sortFilterPopup.querySelectorAll("input[type='checkbox']:not(.nt-sf-select-all)");
                ok = Array.from(cbs).some((cb) => cb.checked);
            }
            else {
                const val = this.sortFilterPopup.querySelector(`#sf-val-${col}`)?.value ?? "";
                ok = val.trim().length > 0;
            }
            const warn = this.sortFilterPopup.querySelector(".nt-sf-warn");
            if (warn)
        ...
        const inputEl = this.sortFilterPopup.querySelector(`#sf-val-${col}`);
        if (inputEl)
            inputEl.addEventListener("input", updateApplyBtn);
        const inputEl2 = this.sortFilterPopup.querySelector(`#sf-val2-${col}`);
        if (inputEl2)
            inputEl2.addEventListener("input", updateApplyBtn);
        const selectAll = this.sortFilterPopup.querySelector(".nt-sf-select-all");
        if (selectAll) {
            selectAll.addEventListener("change", () => {
                const checkboxes = this.sortFilterPopup.querySelectorAll("input[type='checkbox']:not(.nt-sf-select-all)");
                for (const cb of Array.from(checkboxes))
                    cb.checked = selectAll.checked;
                updateApplyBtn();
            });
        }
        const allCheckboxes = this.sortFilterPopup.querySelectorAll("input[type='checkbox']:not(.nt-sf-select-all)");
        ...
        if (applyBtn) {
            ...
            applyBtn.addEventListener("click", () => {
                const dir = this.sortFilterPopup.querySelector(`input[name="sf-sort-${col}"]:checked`)?.value;
                ...
                if (ft === "values") {
                    ...
                    const val = this.sortFilterPopup.querySelector(`#sf-val-${col}`)?.value ?? "";
                    const val2 = this.sortFilterPopup.querySelector(`#sf-val2-${col}`)?.value ?? "";
                    if (!val)
                        return;
                    ...
                    this.view.setFilter(col, { op: ft, value: String(rawVal), value2: val2 ? String(rawVal2) : undefined });
                }
                this.syncView();
        ...
        const close = (ev) => {
            if (this.sortFilterPopup.contains(ev.target))
                return;
    ...
    updateSortIndicators() {
        ...
        for (const btn of Array.from(btns)) {
            ...
            if (entry && this.view.sortStack.length > 1) {
                btn.textContent = `${idx + 1}${entry.asc ? "▲" : "▼"}`;
            }
            else if (entry) {
                btn.textContent = entry.asc ? "▲" : "▼";
            }
            else {
                btn.textContent = "▾";
            }
    ...
    onMouseDown(e) {
        ...
        if (target.classList.contains("nt-resize-handle") && !target.classList.contains("nt-resize-handle-row")) {
            ...
            if (!Number.isNaN(col))
                this.startColResize(col, e.clientX);
            e.preventDefault();
            return;
        }
        // Resize handle (row)
        if (target.classList.contains("nt-resize-handle-row")) {
            ...
            if (!Number.isNaN(row))
                this.startRowResize(row, e.clientY);
            e.preventDefault();
            return;
        }
        ...
        const colHeader = target.closest(".nt-header-cell");
        ...
        const rowHeader = target.closest(".nt-header-cell");
        ...
        if (!target.classList.contains("nt-cell") &&
    ...
    onDoubleClick(e) {
        ...
        if (isBoolean(colDef) || isReadOnly(colDef))
            return;
        this.editor.start(found.row, found.col, colDef, "", false);
    }
    toggleBoolean(row, col) {
    ...
    undo() { this._undoRedo("undo"); }
    /** Вернуть отменённое действие. */
    redo() { this._undoRedo("redo"); }
    /**
    ...
    toggleBold() { this.applyStyle({ bold: !this.getFirstSelectedStyle()?.bold }); }
    /** Инвертировать курсив. */
    toggleItalic() { this.applyStyle({ italic: !this.getFirstSelectedStyle()?.italic }); }
    /** Инвертировать подчёркивание. */
    toggleUnderline() { this.applyStyle({ underline: !this.getFirstSelectedStyle()?.underline }); }
    ...
    showAlignPopup(clientX, clientY) {
        ...
        const makeBtn = (icon, tooltip, action, active) => {
            const btn = document.createElement("div");
            btn.className = "nt-tb-btn";
            if (active)
                btn.classList.add("nt-tb-btn--active");
    ...
    _undoRedo(dir) {
        ...
        for (const c of batch) {
            if (c.colWidth) {
                this.renderer.setColWidth(c.col, c.colWidth.old);
                hasLayout = true;
                continue;
            }
            if (c.rowHeight) {
                this.renderer.setRowHeight(c.row, c.rowHeight.old);
                hasLayout = true;
                continue;
            }
            if (c.oldValue === null)
                this.model.deleteSilent(c.row, c.col);
            else
                this.model.setSilent(c.row, c.col, c.oldValue);
        }
        this.renderer.refreshValues();
        if (hasLayout) {
            this.overlay.update(this.selection);
    ...
    _updateToolbar() {
        const u = document.querySelector(".nt-tb-btn[data-action=undo]");
        const r = document.querySelector(".nt-tb-btn[data-action=redo]");
        const dot = document.querySelector(".nt-tb-dot");
        if (u)
            u.disabled = !this._undoMgr.canUndo;
        if (r)
            r.disabled = !this._undoMgr.canRedo;
        if (dot)
            dot.style.display = this._undoMgr.canUndo ? "block" : "none";
    }
    updateToolbarStyleState() {
        ...
        if (alignBtn)
            alignBtn.classList.toggle(cls, !!(style && (style.align || style.valign)));
    }
    findRowForCell(cellEl) {
    ...
    startFillDrag() {
        ...
        const srcBottom = Math.max(this.selection.start.row, this.selection.end.row);
        ...
        const onMove = (ev) => {
            ...
            if (found.row > srcBottom)
                newBottom = found.row;
            else if (found.row < srcTop)
                newTop = found.row;
            if (found.col > srcRight)
                newRight = found.col;
            else if (found.col < srcLeft)
                newLeft = found.col;
            this.setSelectionNoScroll({
        ...
        const onUp = () => {
            ...
            if (!start || !end)
                return;
            this.applyFill(Math.min(start.row, end.row), Math.max(start.row, end.row), Math.min(start.col, end.col), Math.max(start.col, end.col), srcTop, srcBottom, srcLeft, srcRight);
        };
        window.addEventListener("mousemove", onMove);
        window.addEventListener("mouseup", onUp);
    }
    applyFill(top, bottom, left, right, srcTop, srcBottom, srcLeft, srcRight) {
        ...
        for (let c = left; c <= right; c++) {
            ...
            for (let r = top; r <= bottom; r++) {
                if (r >= srcTop && r <= srcBottom && c >= srcLeft && c <= srcRight)
                    continue;
                const dr = this.toDataRow(r);
                const old = this.model.get(dr, c);
    ...
    copy() {
        ...
        for (let r = sr; r <= er; r++) {
            ...
            for (let c = sc; c <= ec; c++) {
                ...
                row.push(showTypes.includes(colDef?.type ?? "") ? formatCellDisplay(cell.value ?? null, colDef) : (cell.value ?? null));
    ...
    paste() {
        this.clearCopy();
        if (!this.clipboard || !this.selection.start || !this.selection.end)
            return;
    ...
    commitEdit(row, col, value, direction) {
        this.clearCopy();
        const dr = this.toDataRow(row);
        ...
        let nextCol = col;
        if (direction === "enter")
            nextRow = Math.min(row + 1, totalRows - 1);
    ...
    saveColumnWidths() {
        const widths = {};
        for (let c = 0; c < this.renderer.totalCols; c++)
            widths[String(c)] = this.renderer.getColWidth(c);
    ...
    autoScroll(ev) {
        ...
        this.autoScrollDY = 0;
        if (x < margin)
            this.autoScrollDX = -(margin - x);
        else if (x > rect.width - margin)
            this.autoScrollDX = (x - rect.width + margin);
        if (y < margin)
            this.autoScrollDY = -(margin - y);
        else if (y > rect.height - margin)
            this.autoScrollDY = (y - rect.height + margin);
        if (!this.autoScrollDX && !this.autoScrollDY)
            return;
        if (!this.autoScrollTimer) {
            ...
            const step = () => {
                ...
                if (this.lastAutoScrollEv)
                    onMove?.(this.lastAutoScrollEv);
...
function cycle(v, mod) {
    if (mod === 0)
        return 0;
    return ((v % mod) + mod) % mod;
}
/** Подогнать X-позицию попапа под границы экрана. */
function clampPopupX(clientX, el) {
